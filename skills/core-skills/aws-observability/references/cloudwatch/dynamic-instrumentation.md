...
- **Auditing is automatic.** These are control-plane operations, so create/delete calls are recorded
  ...
  removed a breakpoint and when. See `references/cloudwatch/cloudtrail.md` to query that history. For proactive
...
cd "$SKILL_DIR" && python3 scripts/cloudwatch/di_instrumentation.py --print-contract
```
...
python3 scripts/cloudwatch/di_snapshots.py --print-contract
```
...
```bash
cd "$SKILL_DIR" && python3 scripts/cloudwatch/di_instrumentation.py create --region us-west-2 --json-file args.json
```
...
3. Build a compact **call graph** of the suspected area — the caller/callee tree of the functions
   ...
   `references/cloudwatch/dynamic-instrumentation/call-tree-and-directions.md` (node legend: `OK` cleared / `X` issue / `?`
...
- **Location fields** — `file_path`, `code_unit`, `class_name`, `method_name`, and
  ...
  - **Java:** `code_unit` = the package (e.g. `com.amazon.sampleapp`); `class_name` = the
    ...
    **never pass `arg0`/`arg1` to `create`**. Separately, when you later *read* the snapshot, the
    ...
    time. See `references/cloudwatch/dynamic-instrumentation/breakpoint-creation.md`.
...
- An explicit **capture plan**. Required every time:
  ...
  - `ttl_hours = 24` for a BREAKPOINT (omit it and the Application Signals service applies its own
    ...
    `references/cloudwatch/dynamic-instrumentation/breakpoint-creation.md`.
  ...
  - To scope to specific service instances (by version/host/etc.), `attribute_filters` —
    exact-match OTel resource-attribute groups (see `references/cloudwatch/dynamic-instrumentation/breakpoint-creation.md`).
...
4. Interpret status and act:
   ...
   | `ERROR`    | Instrumentation failed     | See ERROR causes in `references/cloudwatch/dynamic-instrumentation/breakpoint-creation.md`; fix the named cause, recreate |
...
- A brief **call-flow tree** of the investigated path, annotating each node (`OK` cleared / `X`
  issue / `?` investigating / `...` pending). See `references/cloudwatch/dynamic-instrumentation/call-tree-and-directions.md` for
