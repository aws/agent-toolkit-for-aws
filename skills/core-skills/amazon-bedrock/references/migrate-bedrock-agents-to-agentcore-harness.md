...
3. Confirm the **source agent's region** here (if the user supplied one, confirm rather than re-ask). The harness must deploy there, but the CLI pins the deploy target from the shell's effective region on the first deploy, so you pass `--region` explicitly (Phase 6; see deploy.md's region trap).
...
Drive the CLI per the approved plan, following [bedrock-agents-to-agentcore-harness/deploy.md](references/bedrock-agents-to-agentcore-harness/deploy.md) end to end: scaffold with `agentcore project create --name <project>` (no `--template`, which yields a managed-harness project), configure `app/<project>/harness.yaml`, `project add gateway` / `gateway-target` / `gateway-connector`, then the **two-phase deploy**. Do **not** use `agentcore project add runtime --type import`. It *does exist*, but it translates the Bedrock Agent into a **code Runtime**, not a Harness, so it is the wrong route for this migration. The pre-1.0 commands `agentcore create`, `agentcore add …`, `agentcore deploy`, and `agentcore create --import` no longer exist (cli.md has the translation table). Pass `--region <source-region>` on the first deploy (deploy.md's region trap). Generate shim code and tool schemas by **adapting** the `.tmpl` templates in `assets/` per [bedrock-agents-to-agentcore-harness/mapping.md](references/bedrock-agents-to-agentcore-harness/mapping.md). Configure the harness/gateway **inbound auth** to match the source's invocation posture discovered in Phase 2 (deploy.md "Inbound auth"). Do not accept the CLI default if it is broader. If deploy fails, surface the error. **Fail loudly**, never silently work around it. In particular, if a shim invocation fails with `AccessDenied`, **do not** run `aws lambda add-permission` on the source. That mutates source-side infra; stop and hand the builder the command (deploy.md "Source-side prerequisites").

Completion criterion: the second `agentcore project deploy` reports success. Verification is deploy-success-only; deeper parity is out of scope.
