# Instrument a Java Lambda Function with ADOT

Add the ADOT Java Lambda layer to a function so it emits OpenTelemetry traces, with Application Signals explicitly disabled.
...
- Modify the function's handler source or its build configuration
...
Preserve any environment variables the function already sets — append to them. If the function already sets `JAVA_TOOL_OPTIONS`, leave it alone: the layer's wrapper attaches the agent itself, so no `-javaagent` flag is needed here.
...
The Java agent adds noticeably more cold-start latency than the other runtimes, because the agent instruments classes as the JVM loads them. If the function is latency-sensitive, mention this to the user — and note that SnapStart and provisioned concurrency are the usual mitigations — rather than silently changing memory or timeout settings.

## Completion

**Tell the user:**

"I've added ADOT Java instrumentation to your Lambda function.

**Changes:**
- Added the `AWSOpenTelemetryDistroJava` layer (version looked up from the ADOT source of truth for your region)
...
**Not changed:** your handler source and build config. No Application Signals policy was added.

**Next steps:**
1. Review the diff and confirm the layer version and region are right.
2. Deploy, then invoke the function once to trigger a cold start. Expect the first invocation to be slower while the agent initializes.
