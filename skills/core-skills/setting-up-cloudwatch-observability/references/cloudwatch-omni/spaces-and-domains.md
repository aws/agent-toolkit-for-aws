...
| Intent | Command |
|---|---|
| Check whether a Domain already exists | `aws cloudwatch-omni list-domains` |
| Create the Domain | `aws cloudwatch-omni create-domain` |
| Read the Domain back | `aws cloudwatch-omni get-domain` |
| Check whether a Space already exists | `aws cloudwatch-omni list-spaces` |
| Create the Space | `aws cloudwatch-omni create-space` |
| Read the Space back | `aws cloudwatch-omni get-space` |
| Delete the Space | `aws cloudwatch-omni delete-space` |
| Delete the Domain | `aws cloudwatch-omni delete-domain` |
...
```
aws___call_aws → aws cloudwatch-omni create-domain --name <domain-name> --identity-providers IAM
```
...
```
aws___call_aws → aws cloudwatch-omni create-domain --name <domain-name> --identity-providers IAM IDC --identity-provider-configuration '{"identityCenterConfiguration": {"identityCenterInstanceArn": "<idc-instance-arn>"}}'
```
...
```
aws___call_aws → aws cloudwatch-omni list-spaces --domain-id <domain-id>
```
...
```
aws___call_aws → aws cloudwatch-omni create-space --name <space-name> --domain-id <domain-id> \
...
```
aws___call_aws → aws cloudwatch-omni get-domain --domain-id <domain-id>

aws___call_aws → aws cloudwatch-omni get-space --space-id <space-id>
```
...
1. **Confirm first, then delete the Space.** Deleting a Space is destructive: the
   ...
   ```
   aws___call_aws → aws cloudwatch-omni delete-space --space-id <space-id>
   ```

2. Delete the Domain. This fails while any Space still exists under it:

   ```
   aws___call_aws → aws cloudwatch-omni delete-domain --domain-id <domain-id>
   ```
