...
- **Idle sessions burning compute** — Runtime bills by vCPU-hour while the session is active, including idle time before `idleRuntimeSessionTimeout` reclaims it. Default timeout is 15 minutes. Call `StopRuntimeSession` when done, or lower the timeout. See `agentcore-harden` Session lifecycle management.
...
- If you need Container, keep the image small (see `agentcore-harden` Initialization time section)

## Cross-references

- For model selection decisions, see [`references/evals.md`](evals.md) Path A (evaluator model choice applies the same way)
- For memory strategy decisions, see [`agentcore-build/references/memory.md`](../../agentcore-build/references/memory.md)
- For log retention (a harden concern), see `agentcore-harden` Observability section
