...
- Giving the agent tools to call external APIs → use `agentcore-connect`
- Deploying the agent → use `agentcore-deploy`
- Debugging agent responses → use `agentcore-debug`
- Securing the agent endpoint for production → use `agentcore-harden` (but this skill covers the client-side auth code)
...
**If not deployed:** "Your agent needs to be deployed before you can call it from an app. Run `agentcore deploy` first, or use the `agentcore-deploy` skill for guidance."
