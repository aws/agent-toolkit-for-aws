...
description: >
  ...
  Not for deploy failures — use agentcore-deploy. Not for performance
  tuning without errors — use agentcore-optimize. Not for VPC
  configuration — use agentcore-build. Not for observability setup or
  missing logs — use agentcore-optimize.
...
- Deploy failures (CDK errors, IAM during deploy) → use `agentcore-deploy`
- Scaffolding a new project → use `agentcore-get-started`
- Measuring quality or setting up monitoring → use `agentcore-optimize`
...
```
/agentcore-debug                      # interactive — describe what's wrong
/agentcore-debug traces               # read and explain recent traces
/agentcore-debug logs                 # search recent logs for errors
/agentcore-debug memory               # diagnose memory recall issues specifically
/agentcore-debug doctor               # check environment prerequisites
```
...
**VPC connectivity issue:** If the agent is in a VPC, check security group rules and route tables. See `agentcore-build` (loads [`references/vpc.md`](../agentcore-build/references/vpc.md)) for VPC-specific debugging.
...
4. **Only after the above, request a quota increase.** See `agentcore-harden` (loads [`references/limits.md`](../agentcore-harden/references/limits.md)) — request it through the Service Quotas console (Amazon Bedrock AgentCore), not by filing a support ticket directly.

See `agentcore-harden` Session lifecycle management section for the full pattern.
...
**Alternative: use the SDK's async task API for fire-and-forget patterns.** If the client doesn't need to wait for the result, register the work via `add_async_task` / `complete_async_task` and return the invocation immediately. See `agentcore-harden` Long-running background tasks section.
...
|---|---|---|
| Memory misconfigured (wrong strategy, namespace, wiring) | `agentcore-build` | Load [`references/memory.md`](../agentcore-build/references/memory.md) |
| Agent invocation from app not working (auth, URL, streaming) | `agentcore-build` | Load [`references/integrate.md`](../agentcore-build/references/integrate.md) |
| VPC connectivity (can't reach RDS, no internet, AZ error) | `agentcore-build` | Load [`references/vpc.md`](../agentcore-build/references/vpc.md) |
| Multi-agent delegation not working | `agentcore-build` | Load [`references/multi-agent.md`](../agentcore-build/references/multi-agent.md) |
| Custom request headers not reaching agent code | `agentcore-build` | Load [`references/request-headers.md`](../agentcore-build/references/request-headers.md) |
| Cross-account invocation from an app in another account | `agentcore-build` | Load [`references/integrate.md`](../agentcore-build/references/integrate.md) (cross-account section) |
| Gateway auth misconfigured (401, wrong auth type) | `agentcore-connect` | Gateway auth matrix |
| Gateway target type question (Lambda vs OpenAPI vs MCP vs API Gateway) | `agentcore-connect` | "What Gateway is and isn't" section |
| Policy denying unexpectedly (Cedar, access denied on tool) | `agentcore-connect` | Load [`references/policy.md`](../agentcore-connect/references/policy.md) |
| Observability not set up (no logs, no traces appearing) | `agentcore-optimize` | Load [`references/observability.md`](../agentcore-optimize/references/observability.md) |
| Cold start / initialization too slow | `agentcore-harden` | Initialization time section |
| Session lifecycle / `maxVms` / `StopRuntimeSession` | `agentcore-harden` | Session lifecycle management section |
| Long-running background tasks being reclaimed | `agentcore-harden` | Long-running background tasks section |
| JWT inbound auth failing (403, `allowedClients`/`allowedAudience`, issuer mismatch) | `agentcore-harden` | Inbound auth section |
| Throttling / quota error / limit increase request | `agentcore-harden` | Load [`references/limits.md`](../agentcore-harden/references/limits.md) |
| Deploy artifact stale or wrong version | `agentcore-deploy` | Redeploy workflow |
