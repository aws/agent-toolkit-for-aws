...
|---|---|
| "Let it look up orders in our database" | `agentcore-connect` (add a gateway target for the order API) |
| "Remember the customer's name between sessions" | `agentcore-build` (loads [`references/memory.md`](../../agentcore-build/references/memory.md)) |
| "Make sure it can't say anything off-policy" | `agentcore-connect` (loads [`references/policy.md`](../../agentcore-connect/references/policy.md)) |
| "Put it on our website" | `agentcore-build` (loads [`references/integrate.md`](../../agentcore-build/references/integrate.md)) |
| "Know if it's actually helpful" | `agentcore-optimize` |
...
For cost-sensitive use cases, consider Bedrock Nova models (e.g., `amazon.nova-micro-v1:0`, `amazon.nova-lite-v1:0`) — significantly cheaper than Claude for simpler extractive tasks. See [`agentcore-optimize/references/cost.md`](../../agentcore-optimize/references/cost.md) for model selection guidance.

For a chatbot that remembers conversations, add `--memory longAndShortTerm` during scaffolding. Memory can also be added later — see [`agentcore-build/references/memory.md`](../../agentcore-build/references/memory.md).
