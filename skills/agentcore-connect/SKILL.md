...
description: >
  ...
  "add tools to agent".
  Not for inbound auth (who can call your agent) — use agentcore-harden.
  Not for debugging agent behavior — use agentcore-debug.
  Not for VPC networking errors (agent can't reach APIs due to VPC) — use
  agentcore-build. Not for creating or hosting a new MCP server project — use
  agentcore-get-started.
...
| Calling another agent via A2A | A2A is HTTP-by-design and has its own auth model | [`agentcore-build/references/multi-agent.md`](../agentcore-build/references/multi-agent.md), not a Gateway target |
...
The app signs the invocation with IAM SigV4 or presents a JWT. See [`agentcore-build/references/integrate.md`](../agentcore-build/references/integrate.md) for the app-side patterns.
...
- "My FastAPI app needs to call my agent" → app is calling the agent → **direct invocation** (not Gateway; use [`agentcore-build/references/integrate.md`](../agentcore-build/references/integrate.md))
...
If the developer needs callers to send custom HTTP headers (tenant IDs, correlation IDs, protocol-specific headers like `A2A-Version`, tracing headers, idempotency keys), the runtime's default is to strip most headers before they reach agent code. Load [`agentcore-build/references/request-headers.md`](../agentcore-build/references/request-headers.md) for the allowlist configuration and prefix pattern.
