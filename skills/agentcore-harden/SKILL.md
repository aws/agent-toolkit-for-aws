...
description: >
  ...
  Not for Cedar tool-restriction policies — use agentcore-connect. Not
  for quality measurement — use agentcore-optimize. Not for outbound
  credential storage or API key wiring — use agentcore-connect. Not for
  A2A agent-to-agent auth — use agentcore-build. Cold start observation
  and diagnosis (not optimization) routes to agentcore-debug.
...
If your project uses `InvokeAgentRuntimeCommand` (see [`agentcore-build/references/integrate.md`](../agentcore-build/references/integrate.md)), audit its IAM permissions separately from `InvokeAgentRuntime`. The two actions have different blast radii: `InvokeAgentRuntimeCommand` is arbitrary shell execution inside a live microVM with the runtime's full execution role — callers can read/write the filesystem, reach any network resource the agent can reach, and access the execution role's credentials.
...
|---|---|
| Calls an external REST API the agent treats as a tool | Front as a Gateway target (`agentcore add gateway-target --type open-api-schema` or `api-gateway`). Load [`agentcore-connect/SKILL.md`](../agentcore-connect/SKILL.md) Path C. |
| Calls an MCP server directly | Front as a Gateway target (`--type mcp-server`). Load [`agentcore-connect/SKILL.md`](../agentcore-connect/SKILL.md) Path A. |
...
See `agentcore-build` (loads [`references/vpc.md`](../agentcore-build/references/vpc.md)) for full VPC configuration guidance.
...
- **Keep the HTTP connection open** with a streaming response and emit periodic heartbeat events. Useful when you want the caller to wait for the result rather than polling. See the SSE keepalive pattern in [`agentcore-debug/SKILL.md`](../agentcore-debug/SKILL.md) ("Connection drops mid-stream" section).
