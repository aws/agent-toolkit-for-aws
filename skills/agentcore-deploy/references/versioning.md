...
This routes 10% of traffic to the new version. Monitor `agentcore-optimize` eval scores and error rates before increasing the weight.
...
- If a rollback is needed because of a specific failure, use `agentcore-debug` to diagnose first
- For staging/production best practices, see `agentcore-harden`
- For running evals against a specific version before cutover, see [`agentcore-optimize/references/evals.md`](../../agentcore-optimize/references/evals.md)
