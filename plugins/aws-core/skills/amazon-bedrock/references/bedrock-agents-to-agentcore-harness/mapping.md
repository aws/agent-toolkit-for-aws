...
Each action group becomes one Gateway **`targetType: "lambda"` code target**, added with `agentcore project add gateway-target --gateway <gw> --target-configuration file://<shim>.json`, with `toolDefinitions` holding **one entry per function/operation**. `agentcore project deploy` then builds the shim Lambda. Exact block + gotchas in [deploy.md](references/bedrock-agents-to-agentcore-harness/deploy.md); follow it.
...
- **`MANAGED`** → then (and only then) check the **retrieval config on the KB association** (`knowledgeBaseConfigurations[].retrievalConfiguration`: reranker, metadata filter, hybrid/search-type override, top-k):
  - **default** retrieval config → native Gateway connector: `agentcore project add gateway-connector --gateway <gw> --name <target-name> --connector bedrock-knowledge-bases --knowledge-base <id>`.
...
- **`AMAZON.CodeInterpreter`**: use the built-in tool: add `- { type: agentcore_code_interpreter, name: <tool-name> }` under `tools` in `app/<harness>/harness.yaml` (no ARN needed. The AWS-managed code interpreter is used. See [deploy.md](references/bedrock-agents-to-agentcore-harness/deploy.md)).

## Mapping memory to managed memory (default)

Use the harness's **managed memory**, which is on by default. The harness auto-provisions an AgentCore Memory instance (semantic + summarization strategies, with the service's default expiry) and loads/saves session history automatically. Check the [harness memory devguide](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/harness-memory.html) or `DescribeHarness` for the current defaults rather than assuming a fixed retention. Keep the scaffold default `memory: { mode: managed }` in `harness.yaml`; do not set `mode: disabled`. This covers the common source case (`SESSION_SUMMARY`) via the built-in `SUMMARIZATION` strategy; customize strategies via `UpdateHarness` only if the source clearly needs more.
...
Set model params in `app/<harness>/harness.yaml` under `model` (`provider: bedrock`, `modelId`, `maxTokens`, `temperature`, `topP`, `apiFormat`), or pass the same keys as JSON to `agentcore project add harness --model`. Some models reject setting temperature and top-p together. Check the target model's inference-parameter constraints (e.g. `aws bedrock get-foundation-model`, or a probe call) and set **only one** if so.

## Mapping idle session TTL (preserve the source's session-lifetime bound)
The source's `idleSessionTTLInSeconds` bounds how long an idle session is retained, a security control (it caps the credential/context leakage and replay window). Read its actual value from the discovery manifest (the source agent's own setting, whatever it is) and preserve it: set `lifecycleConfig.idleRuntimeSessionTimeout` in `harness.yaml` (or `agentcore project add harness --lifecycle-config '{"idleRuntimeSessionTimeout": <seconds>}'`) to the source value. The field accepts 60–28800 seconds; if the source value falls outside that range, use the nearest bound and **classify it degraded in Phase 4** with the concrete delta (e.g. "source set 30s; migrated uses 60s") so the builder decides. Never silently extend the window.

## Guardrail — cannot migrate; surface to the user
Check whether the installed CLI can attach a guardrail to a **bedrock** harness before classifying: look in the scaffolded `harness.yaml` model block and `agentcore project add harness --help` for a guardrail field, and note that `model.additionalParams` (the pass-through) is `lite_llm`-provider only, so a bedrock harness rejects it. If no guardrail path exists in the CLI surface you're running, classify the guardrail **cannot** in the assessment and tell the user the migrated harness will **not** enforce it, recording the source `guardrailIdentifier` + version. Do **not** fake it with a system-prompt mention (that enforces nothing). Enforcing it means applying `ApplyGuardrail` outside the harness, which is out of scope. If the CLI surface you're running exposes a guardrail field, use it and reclassify as clean.

## Mapping the prompt to the system prompt
Fold the agent instruction into `app/<harness>/system-prompt.md` (or `agentcore project add harness --system-prompt`). If `AMAZON.UserInput` was present, include the clarification instruction.
...
Default `model.modelId` to the source agent's `foundationModel`. If the CLI's harness default differs, set it explicitly to match the source (parity).
