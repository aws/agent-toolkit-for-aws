...
description: >
  ...
  Not for adding capabilities to existing projects — use agentcore-build
  or agentcore-connect. Strands vs LangGraph in a migration context routes
  to agentcore-build, not here. Connecting to an existing MCP server routes
  to agentcore-connect, not here.
...
- Environment/prerequisite issues (CLI not found, credentials broken) → use `agentcore-debug`
- Adding capabilities to an existing project (memory, tools, policies) → use `agentcore-build` or `agentcore-connect`
- Migrating an existing Bedrock Agent → use `agentcore-build` (loads [`references/migrate.md`](../agentcore-build/references/migrate.md))
...
> If you're having trouble with installation, I can run the `agentcore-debug` skill (which loads [`references/doctor.md`](../agentcore-debug/references/doctor.md)) to diagnose your environment.
...
- **Memory:** Start with `none` — memory can be added later via `agentcore-build` (loads [`references/memory.md`](../agentcore-build/references/memory.md)) when the developer needs it
...
1. **It will become a silent no-op if you repoint it at a gateway that isn't deployed yet.** The common path is to swap the example endpoint for `os.getenv("AGENTCORE_GATEWAY_<NAME>_URL")`. That env var is only populated after `agentcore deploy`. If the developer repoints and runs `agentcore dev` before deploying, `get_streamable_http_mcp_client()` returns a client with a `None` URL and the agent starts with zero MCP tools — no error, no warning. See the "Local dev gap" section in `agentcore-connect` for the guard pattern: `if not GATEWAY_URL: tools = []`.
...
The reference client code in `agentcore-connect` (Path A) shows the correct pattern for gateway-backed MCP clients once deploy has run.
...
|---|---|---|
| "How do I call it from my app?" | `agentcore-build` | `agentcore fetch access` |
| "I want it to remember things" | `agentcore-build` | `agentcore add memory` |
| "I want it to call external APIs" | `agentcore-connect` | `agentcore add gateway` |
| "I want to restrict what it can do" | `agentcore-connect` | `agentcore add policy-engine` |
| "I want to measure quality" | `agentcore-optimize` | `agentcore add evaluator` |
| "I want to go to production" | `agentcore-harden` | production readiness checklist |
| "I want multiple agents working together" | `agentcore-build` | `agentcore create --protocol A2A` |
| "I need it in a VPC" | `agentcore-build` | `agentcore create --network-mode VPC` |
