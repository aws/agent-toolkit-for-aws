...
description: >
  ...
  Not for debugging errors or crashes — use agentcore-debug. Slow but
...
- Debugging a specific broken agent (wrong answers, errors) → use `agentcore-debug`
- Production security hardening (IAM, auth) → use `agentcore-harden`
...
> "This skill requires an AgentCore project. Use `agentcore-get-started` to create one."
...
- After setting up evals, suggest `agentcore-harden` for production readiness
- If eval results reveal agent issues, suggest `agentcore-debug` for root cause analysis
- If the developer needs to add capabilities first, suggest `agentcore-build`
