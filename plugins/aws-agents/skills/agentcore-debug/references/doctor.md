...
- Creating a new project or getting started → use `agentcore-get-started`
- Deploy failures that aren't environment-related (CDK errors, IAM) → use `agentcore-deploy`
- Agent runtime errors → use `agentcore-debug`
...
All clear? Run `agentcore-get-started` to create your first project.
```
...
- Pointer to `agentcore-get-started` skill when environment is healthy
