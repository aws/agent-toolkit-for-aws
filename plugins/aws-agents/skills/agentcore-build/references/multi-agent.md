...
Passing `"DEFAULT"` as the qualifier calls the live version. To pin to a specific version (staging pin, canary, or rollback), pass a numeric version string instead — see [`agentcore-deploy/references/versioning.md`](../../agentcore-deploy/references/versioning.md) for the full workflow.
