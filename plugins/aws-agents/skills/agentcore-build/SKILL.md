...
description: >
  ...
  Not for connecting to external APIs via Gateway — use agentcore-connect.
  Not for scaffolding a new project — use agentcore-get-started.
  Not for CLI/dev server errors — use agentcore-debug.
...
- Connecting to external tools/APIs via Gateway (OpenAPI specs, Lambda, MCP servers, credentials, policies) → use `agentcore-connect`
- Scaffolding a new project → use `agentcore-get-started`
- Deploying → use `agentcore-deploy`
...
Do not just say "go use agentcore-get-started" and stop — that loses the developer's context about what they actually wanted to do.
...
- If the phrase mentions external APIs, Lambda functions, OpenAPI specs, gateways, credentials, MCP servers, or policies → this is `agentcore-connect`, not build
- If the developer says something is broken (wrong answers, errors, tool failures) → this is `agentcore-debug`, not build
...
- If using `agentcore-harden`, update the IAM policy to scope to the new model ARN
...
Note: `agentcore create --name` (the project name) has a **stricter 23-char limit** and does not allow underscores. That's covered in `agentcore-get-started`; if you see the developer re-running create, flag the 23-char limit specifically.
...
- Cross-references to other skills (agentcore-connect, agentcore-deploy) are included where relevant
