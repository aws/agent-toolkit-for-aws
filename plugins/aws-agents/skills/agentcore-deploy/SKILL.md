...
description: >
  ...
  Not for production hardening — use agentcore-harden. Not for adding
  capabilities before deploy — use agentcore-build or agentcore-connect.
  Not for VPC configuration errors — use agentcore-build.
...
```
/agentcore-deploy                     # interactive — pre-flight check or diagnose failure
/agentcore-deploy preflight           # validate config and IAM before deploying
/agentcore-deploy diagnose            # diagnose a failed deploy (paste error or read logs)
/agentcore-deploy preview             # show what deploy will create without deploying
/agentcore-deploy rollback            # roll back to a previous version
```
