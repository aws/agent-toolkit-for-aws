...
Config file
-----------
Resolved from AGENTS_PAY_CONFIG, else ~/.agents-pay/config.json. Must be a regular
file, owned by the current user, mode 0600, in a directory that is not
group/world-writable, no symlinks (see load_config). One file, two sections:
...
DEFAULT_CONFIG_PATH = Path.home() / ".agents-pay" / "config.json"
...
def load_config(path: str | os.PathLike[str] | None = None) -> dict:
    ...
    a spending credential, and reading it from a 0600 file the agent cannot write
    means a compromised agent cannot point itself at a different, larger-budget
    session. That only holds if the file WINS over the environment — see
    resolve_resource().

    The file is a security control, so one that anyone else can write is treated as
    no config at all. lstat (not stat) so a symlink is rejected, not followed.
    """
    p = Path(
        path
        or os.environ.get("AGENTS_PAY_CONFIG")
        or os.environ.get("X402_POLICY_FILE")  # accepted for continuity
...
def select_accept_entry(challenge: dict, policy: dict) -> dict:
    ...
    for entry in accepts:
        ...
        if amount is None:
            continue
        try:
            if base_units_to_usd(amount) > cap_usd:
                continue
        except PolicyError:
            continue
        return entry
    ...
    *,
    ...
    material = "\x1f".join(  # unit separator: cannot appear in these values
        [
            ...
            str(accept.get("amount", accept.get("maxAmountRequired", ""))),
            purchase_id or "",
        ]
    )
