...
def cmd_init_config(args: argparse.Namespace) -> int:
    path = Path(args.path or DEFAULT_CONFIG)
    ...
    print(
        "\nThe policy section is the ONLY thing that authorizes a payment. The agent "
        "can read this file but cannot write it, and cannot override it at runtime."
    )
    print(
...
def cmd_create_instrument(args: argparse.Namespace) -> int:
    ...
    config_path = Path(args.path or os.environ.get("AGENTS_PAY_CONFIG") or DEFAULT_CONFIG)
