...
copying by hand. The agent reads this file and **never writes it**.
...
multi-tenant use, give each tenant its own config file via `AGENTS_PAY_CONFIG`.

**The config file takes precedence over environment variables.** That ordering is
deliberate: the session ID names the budget being spent, so if a variable could
override the file, anything able to set that variable could redirect spending to a
larger session. Environment variables remain a fallback for containers and Lambda
with no writable home — the weaker mode, because there whatever sets the
environment chooses the session.
