...
For `aws-startup-advisor` that covers startup-focused architecture, cost, security, and AWS Activate guidance, plus migrations to AWS from GCP, Heroku, and AI stacks such as OpenAI and Gemini:
...
| [aws-startup-advisor](plugins/aws-startup-advisor/)           | Personalized AWS guidance for startups, built on patterns from 350,000+ startups. Covers architecture, cost, security, day-one account setup, AWS Activate credits eligibility and startup offers, and migration to AWS from GCP, Heroku, and AI stacks (OpenAI/Gemini SDK rewrites to Amazon Bedrock, agentic systems to AWS-native runtimes).                                    |
